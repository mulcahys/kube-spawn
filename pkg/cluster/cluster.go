...
func (c *Cluster) Start(numberNodes int, cniPluginDir string) error {
	...
	if err := kubeadmInit(kubeadmVersion, masterMachine.Name, multiPrinter.NewWriter(fmt.Sprintf("%s ", masterMachine.Name))); err != nil {
	...
	for _, worker := range workerMachines {
		go func(nodeName string) {
			defer wg.Done()
			if err := kubeadmJoin(kubeadmVersion, masterIP, nodeName, multiPrinter.NewWriter(fmt.Sprintf("%s ", nodeName))); err != nil {
...
func (c *Cluster) RemoveImages(timeout time.Duration) error {
	...
	if len(images) > 0 {
		return fmt.Errorf("failed to remove all images (use `machinectl remove ...` to remove them manually)")
	}
	return nil
}
